...
export const CourseContent = ({className, chapters}: { className?: string, chapters: Chapter[]}):JSX.Element => {
                        ...
                        <div id={`accordion-collapse-body-${index}`} className="hidden" aria-labelledby={`accordion-collapse-body-${index}`}>
                            <div className={`p-5 border ${index !== chapters.length-1 ? 'border-b-0': ''} border-gray-200 dark:border-gray-700 dark:bg-gray-900`}>                    
                                ...
                                {chapter.lectures.map((lecture) => {
                                    ...
                                    if (!lecture.isFile) {
                                        if (lecture.isPreviewAval) {
                                        previewButton = (
                                            <ButtonCourse padding="px-3 py-1" text="text-xs" className="w-full md:w-auto bg-violet-600 hover:bg-violet-700">
                                            Preview
                                            </ButtonCourse>
                                        );
                                        } else {
                                        previewButton = <span>0m 12sec</span>;
                                        }
                                    }

                                    return (
                                        <li className="flex items-center text-base my-2" key={lecture.heading}>
